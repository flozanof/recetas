...
import { IIngredientesGrupo } from '../interfaces/Types';
import { IElaboracion } from '../interfaces/Types';
import RecetaForm from './recetaForm2';
            ...
            <>
                {
                    grpIng.map((grp, grpIndex) => {
                        return (
                            <div key={grpIndex}>
                                {((grpIng.length > 1)) && <Typography paragraph variant="body2">{grp.Grupo}</Typography>}
                                <ul>
...
function getListaElaboracion(elaboracion: IElaboracion[]) {
    return (
        <ul>
...
export default function Receta(props: RecipeProps) {
    const [receta, setReceta] = useState<IReceta>({ Nombre: '', Foto: '', Comensales: 0, Dificultad: 0, TiempoCoccion: 0, TiempoElaboracion: 0, TecnicaElaboracion: '', IngredientesGrupo: [], Elaboracion: [], Notas: ['una', 'dos'] });
    ...
    return (
        <div className="mb-3">
            {editMode
                ? <RecetaForm onSubmit={(p) => { console.log(p.Comensales) }} receta={receta} handleEditMode = {() => handleEditMode()}/>
                :
                <Card sx={{ maxWidth: 345 }}>
                    ...
                    <Collapse in={expanded} timeout="auto" unmountOnExit>
                        <CardContent>
                            ...
                            {getListaIngredientes(receta.IngredientesGrupo)}
                            <Typography key="-2" paragraph variant='subtitle1'>Elaboración:</Typography>
                            {getListaElaboracion(receta.Elaboracion)}
